...
def load_downloads_history():
    """Carrega o histórico de downloads com logs detalhados"""
    if os.path.exists(DOWNLOADS_HISTORY_FILE):
        try:
            with open(DOWNLOADS_HISTORY_FILE, "r", encoding="utf-8") as file:
                ...
                # Log detalhado dos status atuais
                status_count = {}
...
def save_downloads_history(downloads):
    """Salva o histórico de downloads com logs detalhados"""
    debug_logger.info(f"💾 SAVE_HISTORY: Tentando salvar {len(downloads)} downloads")
    ...
    # Log das primeiras 3 entradas para debug
    if downloads:
